...
async def main():
    ...
    destination_input = input(
        "Enter the destination SIP URI or username to call (e.g., sip:allenkisathya@sip.linphone.org or allenkisathya): "
    ).strip()
    ...
    if not sip_trunk_id:
        print(
            "ERROR: You must add LIVEKIT_SIP_OUTBOUND_TRUNK_ID to your .env.local file!"
        )
        return
    ...
    try:
        ...
        print(
            "Keep your 'uv run python src/agent.py dev' running in the other terminal so the agent can join the call."
        )
