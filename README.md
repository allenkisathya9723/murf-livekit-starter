...
Powered by **Murf Falcon TTS** (Fastest Streaming Voice Engine), **Groq Llama-3.3-70B** (Ultra-fast low-latency LLM), **Deepgram Nova-3** (STT), and **LiveKit Agents SDK**.

---

### Challenge Progress & Status
- **Day 1: Get Your Voice Agent Talking** ✅ — Initial setup with Murf Falcon TTS (`Pooja` conversational voice) & LiveKit pipeline (~117ms TTFB).
- **Day 2: Personality, Job & Guardrails** ✅ — Configured JanMitra persona for rural/community health access, strict medical diagnosis & prescription refusals, emergency escalation scripts, Hinglish code-mixing, and automatic first-turn greeting. See [`RED_TEAM.md`](RED_TEAM.md) for adversarial red-team audit results.
